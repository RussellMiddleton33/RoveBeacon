<script lang="ts">
  ...
  function createUserMarker() {
    ...
    userPositionMesh.add(border);

    // Blue dot center
    const dotGeometry = new THREE.CircleGeometry(9, 32);
    const dotMaterial = new THREE.MeshBasicMaterial({ color: 0x4285F4, side: THREE.DoubleSide });
    const dot = new THREE.Mesh(dotGeometry, dotMaterial);
    dot.position.z = 0.2; 
    userPositionMesh.add(dot);

    // Flashlight/Cone of vision indicator (like Apple Maps / Call of Duty radar)
    // Creates a triangular gradient that fades out from the user position
    const coneGroup = new THREE.Group();
    
    // Create multiple layered triangles for gradient effect
    const coneLength = 45;  // Shorter cone
    const coneWidth = 70;   // Wider spread
    const layers = 8;       // Number of gradient layers
    
    for (let i = 0; i < layers; i++) {
        const t = i / (layers - 1); // 0 to 1
        const layerLength = coneLength * (1 - t * 0.3); // Slightly shorter inner layers
        const layerWidth = coneWidth * (1 - t * 0.5);   // Narrower inner layers
        
        const coneShape = new THREE.Shape();
        coneShape.moveTo(0, 0); // Start from center of dot
        ...
        const coneMaterial = new THREE.MeshBasicMaterial({
            ...
            side: THREE.DoubleSide,
            transparent: true,
            opacity: 0.2 * (1 - t * 0.7), // More transparent overall
            depthWrite: false
        });
        
        const coneMesh = new THREE.Mesh(coneGeometry, coneMaterial);
        coneMesh.position.z = 0.1 + t * 0.01; // Stack layers slightly
        coneGroup.add(coneMesh);
    }
    
    // Add a brighter core/edge highlight
    const coreShape = new THREE.Shape();
    coreShape.moveTo(0, 0); // Start from center
    ...
    userArrowMesh.visible = false; // Hidden until we have heading and moving
  ...
  function animate() {
    ...
    // Accuracy-based Pulse Animation
    // Accuracy is in meters - larger number = less accurate
    // Scale the glow ring to represent accuracy radius
    if (userGlowMesh && userPositionMesh?.visible) {
        pulsePhase += 0.03;
        
        // Base scale from accuracy (clamp between 5m and 100m for visual purposes)
        // At 5m accuracy: small ring, at 100m: large ring
        const clampedAccuracy = Math.max(5, Math.min(100, currentAccuracy || 10));
        const accuracyScale = clampedAccuracy / 10; // 5m = 0.5x, 50m = 5x, 100m = 10x
        
        // Add subtle pulse on top of accuracy
        ...
        // More transparent when less accurate
    ...
    if (userArrowMesh && currentHeading !== null) {
        // Heading from GPS: 0° = North, 90° = East (clockwise)
        // In our scene: +Y = North, +X = East
        // Three.js rotation.z: positive = counter-clockwise
        // So heading 0° (North/+Y) needs rotation 0
        // Heading 90° (East/+X) needs rotation -90° (-PI/2)
        // Formula: rotation = -heading (in radians)
        ...
        userArrowMesh.rotation.z += diff * 0.15; // Slightly faster interpolation
    }
  ...
  function watchLocation() {
    ...
    navigator.geolocation.watchPosition(
      (position) => {
        ...
        if (isWalking && heading !== null && !isNaN(heading)) {
            currentHeading = heading;
            if (userArrowMesh) {
                userArrowMesh.visible = true;
            }
        } else {
            // Hide arrow when stationary or slow
            if (userArrowMesh) {
